...
作成日: 2026-07-17
変更履歴: 2026-07-17 サマリーを全体 1 本からリポジトリごとに変更（ユーザー指示）。
生成・キャッシュ・payload（`todoSummaries: {repo, summary}[]`）・表示（タグ + 件数 + 1〜2 文）
すべてリポジトリ単位。あるリポジトリだけ TODO.md が変わった朝は、そのリポジトリの分だけ再生成される。
